...
type Page = "explore"|"cinema"|"audio"|"image"|"marketing"|"influencer"|"canvas"|"apps"|"library"|"login"|"pricing";
...
const UGC_TYPES    = ["UGC","Product Demo","Founder Story","Testimonial","Unboxing","Problem / Solution","Luxury Ad","Meme Ad"];
const HOOKS        = ["Strong Hook","Question Hook","Pain Point","Before / After","Shocking Claim","Social Proof","Fast Benefit"];
const SETTINGS_AD  = ["Home","Studio","Street","Office","Gym","Luxury Apartment","Store","Outdoor","Car","Beach"];
  ...
  return (
    <div style={{display:"flex",alignItems:"center",gap:8}}>
      <div style={{
        ...
        background:`linear-gradient(135deg,${C.magenta},${C.magentaD})`,
        display:"flex",alignItems:"center",justifyContent:"center",
        fontWeight:900,fontSize:size*0.5,color:"#fff",letterSpacing:-1,
        flexShrink:0,
      }}>L</div>
      <span style={{color:C.text,fontWeight:800,fontSize:size*0.5,letterSpacing:-.5,lineHeight:1}}>
        Lumen<span style={{color:C.magenta}}>field</span>
      </span>
    </div>
  );
}
...
function Nav({cur,setPage}:{cur:Page;setPage:(p:Page)=>void}) {
  ...
  const items = [
    ...
    {l:"Marketing",   p:"marketing" as Page},
    {l:"Cinema",      p:"cinema"    as Page},
  ...
  return (
    <nav style={{
      ...
      background:"rgba(5,5,5,.95)",backdropFilter:"blur(16px)",
      borderBottom:`1px solid ${C.border}`,
      height:57,display:"flex",alignItems:"center",padding:"0 20px",gap:4,
    }} onMouseLeave={()=>setMega(null)}>

      {/* Logo */}
      <button onClick={()=>{setPage("explore");setMega(null);}}
        style={{background:"none",border:"none",cursor:"pointer",marginRight:16,flexShrink:0}}>
        <Logo/>
      </button>
        ...
        {items.map(item=>(
          ...
          >
            ...
            {(item as {badge?:string}).badge&&(
              <span style={{background:C.magenta,color:"#fff",fontSize:8,fontWeight:800,padding:"2px 6px",borderRadius:4,letterSpacing:.8,textTransform:"uppercase"}}>
                {(item as {badge?:string}).badge}
              </span>
            )}
          </button>
      ...
      {mega && (() => {
        ...
        return (
          <div style={{
            ...
            background:"rgba(5,5,5,.98)",backdropFilter:"blur(20px)",
            borderBottom:`1px solid ${C.border}`,
            padding:"28px 48px",display:"flex",gap:64,
            ...
            <div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"5px 28px",marginTop:12}}>
                {src.features.map(f=>(
                  <button key={f} style={{background:"none",border:"none",color:C.text2,fontSize:12,textAlign:"left",padding:"4px 0",cursor:"pointer",transition:"color .12s"}}
                    onMouseEnter={e=>(e.currentTarget.style.color=C.magentaL)}
            ...
            <div>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:3,marginTop:12}}>
                {src.models.map(m=>(
                  <button key={m} style={{background:"none",border:"none",color:C.text2,fontSize:12,textAlign:"left",padding:"4px 0",cursor:"pointer",display:"flex",alignItems:"center",gap:8,transition:"color .12s"}}
    ...
    <div style={{minHeight:"100vh",paddingTop:57,background:C.bg,overflowX:"hidden"}}>
      ...
      <section style={{position:"relative",padding:"100px 48px 80px",textAlign:"center",overflow:"hidden"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          <h1 style={{fontSize:"clamp(38px,6vw,82px)",fontWeight:900,color:C.text,letterSpacing:-3,lineHeight:1.02,marginBottom:20,maxWidth:820,margin:"0 auto 20px"}}>
            Create stunning{" "}
            <span style={{color:C.magenta,display:"inline-block",minWidth:180,transition:"all .3s"}}>
              {words[tick%words.length]}
            </span>
            <br/>with one command.
          </h1>

          <p style={{color:C.text2,fontSize:16,maxWidth:480,margin:"0 auto 36px",lineHeight:1.65}}>
            All models. All modalities. One platform built for creators who move fast.
          </p>

          {/* CTAs */}
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            <button className="btn-m" onClick={()=>setPage("cinema")} style={{padding:"14px 34px",fontSize:14,animation:"glow 3s ease-in-out infinite"}}>
              Launch Studio →
            </button>
            <button className="btn-g" onClick={()=>setPage("apps")} style={{padding:"14px 34px",fontSize:14}}>
              Explore Apps
            </button>
          </div>
      ...
      <section style={{padding:"0 48px 64px",maxWidth:1280,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(168px,1fr))",gap:10}}>
          {featured.map(f=>(
            <button key={f.name} onClick={()=>setPage(f.p)} className="card"
              style={{padding:"20px 16px",textAlign:"left",cursor:"pointer",background:C.surface,border:`1px solid ${C.border}`}}
              onMouseEnter={e=>{const el=e.currentTarget as HTMLButtonElement;el.style.borderColor=f.a;el.style.background=`${f.a}08`;}}
              onMouseLeave={e=>{const el=e.currentTarget as HTMLButtonElement;el.style.borderColor=C.border;el.style.background=C.surface;}}
            >
      ...
      <section style={{padding:"0 48px 80px",maxWidth:1280,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(140px,1fr))",gap:8,marginTop:16}}>
          {PRESETS.map(p=>(
            <button key={p} onClick={()=>setPage("cinema")}
              style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,padding:"14px 10px",color:C.text3,fontSize:10,fontWeight:800,letterSpacing:1,textTransform:"uppercase",cursor:"pointer",transition:"all .15s"}}
              onMouseEnter={e=>{const el=e.currentTarget as HTMLButtonElement;el.style.background=`${C.magenta}10`;el.style.color=C.magentaL;el.style.borderColor=`${C.magenta}50`;}}
              onMouseLeave={e=>{const el=e.currentTarget as HTMLButtonElement;el.style.background=C.surface;el.style.color=C.text3;el.style.borderColor=C.border;}}
            >{p}</button>
          ))}
        </div>
      </section>
      ...
      <section style={{margin:"0 48px 80px",background:C.surface,border:`1px solid ${C.border}`,borderRadius:16,padding:"52px",textAlign:"center",position:"relative",overflow:"hidden",maxWidth:1184,marginLeft:"auto",marginRight:"auto"}}>
        ...
        <h2 style={{fontSize:"clamp(22px,3vw,42px)",fontWeight:800,letterSpacing:-1.5,marginBottom:12}}>
          Ready to generate the impossible?
        </h2>
        <p style={{color:C.text2,fontSize:14,marginBottom:28}}>Join 2 million creators. No credit card required.</p>
        <button className="btn-m" onClick={()=>setPage("login")} style={{padding:"14px 36px",fontSize:14}}>
          Start for free →
        </button>
      </section>
    </div>
...
function CinemaPage() {
  ...
  const bgs = [
    "linear-gradient(135deg,#0d0019,#1a003a,#0d0019)",
    "linear-gradient(135deg,#000d1a,#001f3f,#000d1a)",
    "linear-gradient(135deg,#190000,#3a0018,#190000)",
  ];
  ...
  return (
    <div style={{display:"flex",height:"100vh",paddingTop:57,background:C.bg}}>
      ...
      <aside style={{width:200,flexShrink:0,borderRight:`1px solid ${C.border}`,padding:"16px 10px",display:"flex",flexDirection:"column",gap:2,background:C.bg}}>
        ...
        <button style={{background:"none",border:`1px solid ${C.border2}`,borderRadius:7,color:C.text3,fontSize:10,padding:"7px 12px",cursor:"pointer"}}>
          💎 Pricing — 30% OFF
        </button>
      </aside>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{
          ...
          display:"flex",alignItems:"center",justifyContent:"center",
          padding:40,overflow:"auto",position:"relative",
        }}>
          {!loading && results.length===0 && (
            <div style={{textAlign:"center"}} className="fade-up">
              <div style={{fontSize:"clamp(18px,4vw,52px)",fontWeight:900,color:C.border2,letterSpacing:-2,lineHeight:1,marginBottom:4,WebkitTextStroke:`1px ${C.border}`}}>
                CREATE YOUR FIRST PROJECT.
              </div>
              <div style={{fontSize:"clamp(18px,4vw,52px)",fontWeight:900,background:`linear-gradient(90deg,${C.magenta},${C.magentaL})`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:-2,lineHeight:1}}>
              ...
              <div style={{marginBottom:24}}>
                {LOAD_STEPS.map((s,i)=>(
                  <div key={s} style={{display:"flex",alignItems:"center",gap:10,justifyContent:"center",marginBottom:12,color:i<step?C.text2:C.border2,fontSize:13,transition:"color .3s"}}>
                    <div style={{width:18,height:18,borderRadius:"50%",border:`1.5px solid ${i<step?C.magenta:C.border2}`,background:i<step?C.magenta:"transparent",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#fff",transition:"all .3s",flexShrink:0}}>
                      {i<step?"✓":""}
                    </div>
                    {s}
                  </div>
          ...
          {results.length>0 && (
            <div style={{display:"flex",gap:14,flexWrap:"wrap",justifyContent:"center"}} className="fade-in">
              {results.map((_,i)=>(
                <div key={i} style={{width:240,height:152,background:bgs[i],borderRadius:10,border:`1px solid ${C.border}`,position:"relative",overflow:"hidden",cursor:"pointer",transition:"transform .2s"}}
        ...
        <div style={{background:C.surface,borderTop:`1px solid ${C.border}`,padding:"14px 16px"}}>
          ...
          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>
            ...
            <div style={{position:"relative"}}>
              <button onClick={()=>setModelOpen(!modelOpen)}
                style={{background:C.surface2,border:`1px solid ${C.border}`,borderRadius:8,color:C.text,padding:"9px 12px",fontSize:11,cursor:"pointer",display:"flex",alignItems:"center",gap:6,fontWeight:600,transition:"border-color .15s"}}
                  ...
                  {["Cinematic","Featured"].map(group=>(
                    <div key={group}>
                      ...
                      {CINEMA_MODELS.filter(m=>m.group===group).map(m=>(
                        <button key={m.name} onClick={()=>{setModel(m.name);setModelOpen(false);}}
          ...
          <div style={{display:"flex",gap:10,marginTop:10,alignItems:"stretch",flexWrap:"wrap"}}>
            ...
            <div style={{flex:1}}/>
            <button className="btn-m" onClick={generate} disabled={loading}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column"}}>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:C.bg,backgroundImage:`radial-gradient(ellipse 50% 40% at 50% 50%,rgba(232,0,111,.05),transparent)`,position:"relative",overflow:"hidden"}}>
          ...
          <div style={{textAlign:"center",zIndex:1}} className="fade-up">
            <Lbl s="Audio Studio"/>
            <h1 style={{fontSize:"clamp(24px,4vw,52px)",fontWeight:900,letterSpacing:-2,marginBottom:10,marginTop:12,lineHeight:1.05}}>
              Ready to give your<br/>scene a voice?
            </h1>
          ...
          {waveform.length>0 && (
            <div className="fade-in" style={{marginTop:36,background:C.surface,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 20px",display:"flex",gap:2,alignItems:"center",zIndex:1}}>
              ...
              {waveform.map((h,i)=>(
                <div key={i} style={{width:2.5,borderRadius:2,height:`${h*.6}px`,background:`linear-gradient(to top,${C.magenta},${C.magentaL})`,transition:"height .3s"}}/>
              ))}
              <span style={{color:C.text3,fontSize:10,marginLeft:10,flexShrink:0}}>
                0:0{Math.ceil(text.split(" ").length/2.5)} · {audioModel}
              </span>
            </div>
          )}
        </div>
        ...
        <div style={{background:C.surface,borderTop:`1px solid ${C.border}`,padding:"16px 20px",display:"flex",gap:14,alignItems:"flex-end",flexWrap:"wrap"}}>
          ...
          <div style={{flex:1,minWidth:200}}>
            ...
            <textarea className="inp" value={text} onChange={e=>setText(e.target.value)}
              placeholder="Type your narration or script here…"
              rows={4} style={{resize:"none",marginTop:6,lineHeight:1.5}}/>
          </div>
          ...
          <button className="btn-m" onClick={generate} disabled={loading} style={{padding:"12px 28px",alignSelf:"flex-end"}}>
...
function MarketingPage() {
  ...
  return (
    <div style={{display:"flex",height:"100vh",paddingTop:57,background:C.bg}}>
      <aside style={{width:200,flexShrink:0,borderRight:`1px solid ${C.border}`,padding:"16px 10px",display:"flex",flexDirection:"column",gap:2,background:C.bg}}>
        <div style={{fontSize:12,color:C.text,fontWeight:700,padding:"6px 10px",marginBottom:6}}>Marketing Studio</div>
        {["New Project","Search","URL to Ad","Ad Reference","All Generations"].map(s=><button key={s} className="sb">{s}</button>)}
        <div style={{flex:1}}/>
        <Hr/>
      ...
      <div style={{flex:1,overflow:"auto"}}>
        <div style={{padding:"36px 40px 28px",borderBottom:`1px solid ${C.border}`,position:"relative",overflow:"hidden"}}>
          ...
          <h1 style={{fontSize:"clamp(22px,3.5vw,44px)",fontWeight:900,letterSpacing:-1.5,marginTop:10,lineHeight:1.05}}>
            TURN ANY PRODUCT<br/>INTO A VIDEO AD
          </h1>
          <p style={{color:C.text3,fontSize:13,marginTop:8}}>Upload product · choose style · get 9:16 ads in seconds</p>
        </div>

        <div style={{padding:"28px 40px",maxWidth:860}}>
          <div style={{display:"flex",gap:0,marginBottom:20,background:C.bg,borderRadius:7,padding:3,width:"fit-content",border:`1px solid ${C.border}`}}>
          ...
          <textarea className="inp" value={prompt} onChange={e=>setPrompt(e.target.value)}
            ...
            rows={3} style={{resize:"none",marginBottom:14,lineHeight:1.5}}/>

          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(160px,1fr))",gap:10,marginBottom:14}}>
            <Sel label="Ad Type"    options={UGC_TYPES}   value={ugc}     onChange={setUgc}/>
          ...
          <div style={{display:"flex",gap:10,marginBottom:20,flexWrap:"wrap",alignItems:"stretch"}}>
            ...
            <div style={{flex:1}}/>
            <button className="btn-m" onClick={generate} disabled={loading} style={{padding:"0 32px",minHeight:48,fontSize:13}}>
          ...
          {results.length>0 && (
            <div style={{display:"flex",gap:14,flexWrap:"wrap"}} className="fade-in">
              {results.map((_,i)=>(
                <div key={i} style={{width:150,height:266,borderRadius:12,background:adBgs[i],border:`1px solid ${C.border2}`,position:"relative",overflow:"hidden",cursor:"pointer",transition:"transform .2s"}}
  ...
  const Grid = ({label,field,opts}:{label:string;field:string;opts:string[]}) => (
    <div style={{marginBottom:18}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
        {opts.map(o=>(
          <button key={o} onClick={()=>up(field,o)} style={{background:sel[field]===o?`${C.magenta}18`:C.surface2,border:`1px solid ${sel[field]===o?C.magenta:C.border}`,color:sel[field]===o?C.magentaL:C.text3,borderRadius:6,padding:"5px 10px",fontSize:11,cursor:"pointer",transition:"all .12s"}}>{o}</button>
        ))}
      </div>
    </div>
  );
  ...
  return (
    <div style={{display:"flex",height:"100vh",paddingTop:57,background:C.bg}}>
      <aside style={{width:160,flexShrink:0,borderRight:`1px solid ${C.border}`,padding:"14px 10px",display:"flex",flexDirection:"column",gap:6,background:C.bg}}>
        ...
        <button style={{background:"none",border:`1px dashed ${C.border2}`,color:C.text3,borderRadius:8,padding:"8px",fontSize:10,cursor:"pointer",marginTop:4,transition:"all .15s"}}
          onMouseEnter={e=>{e.currentTarget.style.borderColor=C.magenta;e.currentTarget.style.color=C.magentaL;}}
      ...
      <div style={{width:268,flexShrink:0,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20,background:`radial-gradient(ellipse at center,${C.magenta}04,transparent)`}}>
        <div style={{width:196,height:256,borderRadius:16,background:done?`linear-gradient(160deg,#0d0019,#1a003a,#0d0019)`:C.surface,border:`1px solid ${done?C.magenta:C.border}`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden",transition:"all .4s"}}>
          {loading && (
            <div style={{textAlign:"center"}}>
              <div style={{width:28,height:28,border:`2px solid ${C.magenta}`,borderTopColor:"transparent",borderRadius:"50%",animation:"spin 1s linear infinite",margin:"0 auto 8px"}}/>
              <div style={{color:C.text3,fontSize:11}}>Generating…</div>
            </div>
          )}
          {done && !loading && (
            <>
              <div style={{width:72,height:92,borderRadius:36,background:`linear-gradient(160deg,${C.magenta},${C.magentaL})`,position:"absolute",top:"18%"}}/>
              <div style={{position:"absolute",bottom:0,left:0,right:0,height:"40%",background:"linear-gradient(to top,#0d0019,transparent)"}}/>
              <div style={{position:"absolute",bottom:14,color:C.text,fontSize:11,textAlign:"center"}}>
                <div style={{fontWeight:700}}>Lumenfield Character</div>
                <div style={{color:C.text3,fontSize:9}}>{sel.char} · {sel.gender}</div>
              </div>
            </>
          )}
        ...
        <button className="btn-m" onClick={generate} disabled={loading}
          style={{marginTop:16,width:"100%",padding:"11px",animation:!loading&&!done?"glow 2.5s ease-in-out infinite":"none"}}>
          {loading?"Generating…":done?"Regenerate →":"Generate Influencer →"}
        </button>
      </div>
...
function CanvasPage() {
  ...
  return (
    <div style={{height:"100vh",paddingTop:57,background:"#020202",position:"relative",overflow:"hidden"}}>
      ...
      {[
        {top:"15%",left:"8%",  w:130,h:88, bg:"linear-gradient(135deg,#0d1a2e,#1e3a5f)"},
        {top:"22%",right:"10%",w:110,h:70, bg:`linear-gradient(135deg,${C.magentaD},#1a003a)`},
        {top:"62%",left:"6%",  w:100,h:76, bg:"linear-gradient(135deg,#0a1a28,#1a3a4f)"},
        {top:"70%",right:"14%",w:90, h:60, bg:"linear-gradient(135deg,#1a0d00,#3a1a00)"},
      ].map((c,i)=>(
      ...
      <div style={{position:"absolute",top:"18%",right:"22%",background:"#fff",color:"#111",borderRadius:10,padding:"6px 12px",fontSize:12,fontWeight:600,boxShadow:"0 4px 20px rgba(0,0,0,.5)"}}>
        That&apos;s stunning! 🎉
      </div>
      <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",textAlign:"center",zIndex:10}}>
        <div style={{border:`2px solid ${C.magenta}40`,borderRadius:14,padding:"22px 36px",background:"rgba(5,5,5,.9)",backdropFilter:"blur(10px)",marginBottom:20}}>
          <div style={{fontSize:"clamp(12px,2vw,18px)",fontWeight:900,color:C.magenta,letterSpacing:.5}}>
            GENERATE STUNNING MEDIA WITH AI CANVAS
          </div>
        </div>
        {!created ? (
          <div style={{background:"rgba(10,10,10,.96)",border:`1px solid ${C.border}`,borderRadius:14,padding:"28px 44px",backdropFilter:"blur(8px)"}}>
          ...
          <div style={{color:C.magenta,fontSize:14,fontWeight:700}}>✓ Canvas created!</div>
        )}
      </div>
      <div style={{position:"absolute",bottom:72,left:"50%",transform:"translateX(-50%)",display:"flex",gap:8}}>
        {["Long Video","Seedance 2.0","Extend Video","Storyboard","Product Board","Character Builder"].map(t=>(
          <div key={t} style={{background:"rgba(10,10,10,.88)",border:`1px solid ${C.border}`,borderRadius:7,padding:"7px 12px",fontSize:10,color:"#3a7abf",cursor:"pointer",whiteSpace:"nowrap",backdropFilter:"blur(8px)",transition:"all .15s"}}
            onMouseEnter={e=>{e.currentTarget.style.borderColor=C.magenta;e.currentTarget.style.color=C.magentaL;}}
            onMouseLeave={e=>{e.currentTarget.style.borderColor=C.border;e.currentTarget.style.color="#3a7abf";}}
          >{t}</div>
        ))}
      </div>
      <div style={{position:"absolute",bottom:20,right:20,background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",display:"flex",alignItems:"center",gap:12,boxShadow:"0 8px 24px rgba(0,0,0,.6)"}}>
        <div>
          <div style={{color:"#f59e0b",fontSize:11,fontWeight:700}}>Credits running low!</div>
          <div style={{color:C.text3,fontSize:10}}>Over 90% already used</div>
        </div>
              ...
              {(APPS[group]??[]).map(app=>(
                <div key={app.name} className="card" style={{padding:0,overflow:"hidden"}}>
                  ...
                  <div style={{padding:"12px 14px"}}>
                    ...
                    <button onClick={()=>setPage("cinema")}
                      style={{width:"100%",background:`${acc[group]||C.magenta}12`,border:`1px solid ${acc[group]||C.magenta}40`,color:acc[group]||C.magenta,borderRadius:6,padding:"6px 0",fontSize:11,cursor:"pointer",fontWeight:700,transition:"all .15s"}}
                      onMouseEnter={e=>{e.currentTarget.style.background=acc[group]||C.magenta;e.currentTarget.style.color="#fff";}}
                      onMouseLeave={e=>{e.currentTarget.style.background=`${acc[group]||C.magenta}12`;e.currentTarget.style.color=acc[group]||C.magenta;}}
                    >Try Now →</button>
                  </div>
                </div>
              ))}
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:22,flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:6}}>
          {(["grid","list"] as const).map(v=>(
            <button key={v} onClick={()=>setView(v)} style={{background:view===v?C.surface2:"transparent",border:`1px solid ${C.border}`,color:view===v?C.text:C.text3,borderRadius:6,padding:"6px 12px",fontSize:11,cursor:"pointer"}}>{v==="grid"?"⊞":"≡"} {v}</button>
          ))}
        </div>
      </div>
...
function LoginPage({setPage}:{setPage:(p:Page)=>void}) {
  ...
  const bgs = [
    `linear-gradient(160deg,#1a0030,${C.magentaD},#0d0019)`,
    "linear-gradient(160deg,#001020,#003060,#001020)",
    "linear-gradient(160deg,#200010,#600030,#200010)",
    "linear-gradient(160deg,#001020,#002040,#001020)",
    "linear-gradient(160deg,#102000,#205000,#102000)",
    "linear-gradient(160deg,#001020,#003030,#001020)",
  ];
    ...
    <div style={{display:"flex",height:"100vh",paddingTop:57,background:C.bg}}>
      <div style={{width:"42%",minWidth:340,flexShrink:0,display:"flex",flexDirection:"column",justifyContent:"center",padding:"40px 56px",borderRight:`1px solid ${C.border}`}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:9}}>
          {[
            {icon:"G",label:"Continue with Google",    bg:"#fff",    fg:"#1a1a1a",border:"#e0e0e0"},
            {icon:"⌘",label:"Continue with Apple",     bg:"#111",    fg:"#fff",   border:C.border2},
            {icon:"M",label:"Continue with Microsoft", bg:"#0078d4", fg:"#fff",   border:"#0078d4"},
            {icon:"✉",label:"Continue with Email",     bg:C.surface, fg:C.text2,  border:C.border2},
          ].map(b=>(
            ...
            >
              <span style={{fontSize:15,width:18,textAlign:"center"}}>{b.icon}</span>
              {b.label}
            </button>
          ))}
        </div>
        ...
        <p style={{color:C.text3,fontSize:10,lineHeight:1.7}}>
          By continuing you agree to our Terms of Service and Privacy Policy. Lumenfield is an original, independently built platform.
        </p>
      </div>
      ...
      <div style={{flex:1,background:bgs[tab],position:"relative",transition:"background .5s ease",overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",bottom:28,left:0,right:0,display:"flex",justifyContent:"center",gap:6,flexWrap:"wrap",padding:"0 16px"}}>
          {tabs.map((t,i)=>(
            <button key={t} onClick={()=>setTab(i)} style={{background:tab===i?C.magenta:"rgba(255,255,255,.1)",border:`1px solid ${tab===i?C.magenta:"rgba(255,255,255,.2)"}`,color:"#fff",borderRadius:20,padding:"5px 14px",fontSize:11,cursor:"pointer",fontWeight:tab===i?700:400,backdropFilter:"blur(8px)",transition:"all .2s"}}>{t}</button>
          ))}
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{minHeight:"100vh",paddingTop:57,background:C.bg}}>
      <div style={{textAlign:"center",padding:"72px 48px 48px"}}>
        <div style={{display:"inline-block",background:`${C.magenta}15`,border:`1px solid ${C.magenta}40`,borderRadius:20,padding:"4px 14px",fontSize:11,color:C.magentaL,marginBottom:20,fontWeight:700,letterSpacing:.5}}>
          🎉 30% OFF — Limited Time
        </div>
        ...
        {plans.map(plan=>(
          <div key={plan.name} style={{background:plan.highlight?`linear-gradient(160deg,${C.magenta}10,${C.magentaD}08)`:C.surface,border:`1px solid ${plan.highlight?C.magenta:C.border}`,borderRadius:14,padding:"30px 26px",flex:1,minWidth:240,maxWidth:300,position:"relative",boxShadow:plan.highlight?`0 0 40px ${C.magenta}20`:"none"}}>
            {(plan as {badge?:string}).badge && (
              <div style={{position:"absolute",top:-11,left:"50%",transform:"translateX(-50%)",background:C.magenta,color:"#fff",fontSize:9,fontWeight:800,letterSpacing:1,padding:"3px 12px",borderRadius:10}}>
                {(plan as {badge?:string}).badge}
              </div>
            )}
            ...
            <button className={plan.highlight?"btn-m":"btn-g"} style={{width:"100%",padding:"11px"}}>
...
function ImagePage() {
  ...
  const imgBgs = [
    "linear-gradient(135deg,#0d0019,#3a0060,#0d0019)",
    "linear-gradient(135deg,#000d1a,#003060,#000d1a)",
    "linear-gradient(135deg,#190000,#500010,#190000)",
    "linear-gradient(135deg,#001900,#005020,#001900)",
  ];
  const generate = async () => {
    if (!prompt.trim()) return;
    setLoading(true); setImgs([]);
    await new Promise(r=>setTimeout(r,1800));
    setLoading(false); setImgs([1,2,3,4]);
  };
  return (
    <div style={{display:"flex",height:"100vh",paddingTop:57,background:C.bg}}>
      ...
      <div style={{flex:1,padding:"28px 36px",overflow:"auto"}}>
        ...
        <div style={{display:"flex",gap:10,marginBottom:14,flexWrap:"wrap",alignItems:"flex-end"}}>
          <div style={{flex:1,minWidth:240}}>
            <textarea className="inp" value={prompt} onChange={e=>setPrompt(e.target.value)}
              placeholder="Describe the image you want to create…" rows={2} style={{resize:"none"}}/>
          </div>
          <Sel label="Model" options={IMG_MODELS} value={model} onChange={setModel}/>
          <button className="btn-m" onClick={generate} disabled={loading} style={{padding:"10px 24px"}}>
            {loading?"Generating…":"Generate"}
          </button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>
          {loading && Array.from({length:4}).map((_,i)=>(
            <div key={i} className="sk" style={{aspectRatio:"1",borderRadius:10,animationDelay:`${i*.12}s`}}/>
          ))}
          {imgs.map((_,i)=>(
            <div key={i} style={{aspectRatio:"1",background:imgBgs[i],borderRadius:10,border:`1px solid ${C.border}`,cursor:"pointer",transition:"transform .2s"}}
              onMouseEnter={e=>(e.currentTarget.style.transform="scale(1.02)")}
              onMouseLeave={e=>(e.currentTarget.style.transform="scale(1)")}
            />
          ))}
