...
const cards = [
  ...
  { title: "Video Generation", desc: "Turn ideas into cinematic short videos and ads.", icon: "▯", badge: "Soon", type: "video", gradient: "linear-gradient(135deg, #4da3ff, #71ff00)" },
  ...
  return (
    <main style={styles.shell}>
      <aside style={styles.sidebar}>
        ...
        <nav style={styles.nav}>{menu.map((item, index) => <button key={item} style={styles.navItem} onClick={() => index === 0 && setPrompt("")}><span style={styles.navIcon}>{index === 0 ? "+" : index === 1 ? "⌕" : "▦"}</span>{item}{item === "Marketplace" && <span style={styles.newBadge}>NEW</span>}</button>)}</nav>
        <div style={styles.taskTitle}>Tasks</div>
        <div style={styles.emptyTask}><div style={styles.emptyIcon}>+</div><b>{tasks[0]}</b><span>Create one to get started</span></div>
        <div style={styles.priceBox}><span>Pricing</span><b>30% OFF</b></div>
      </aside>

      <section style={styles.content}>
        <header style={styles.topbar}><button style={styles.roundButton}>□</button><button style={styles.shortcut}>⌘ Shortcuts</button></header>
        ...
        <section style={styles.hero}>
          ...
          <div style={styles.promptBox}>
            ...
            <div style={styles.promptBottom}>
              ...
              <button style={styles.askButton} onClick={() => runPrompt()} disabled={loading}>{loading ? "…" : "↑"}</button>
            </div>
          </div>
...
const styles: Record<string, CSSProperties> = {
  shell: { minHeight: "100vh", display: "grid", gridTemplateColumns: "220px 1fr", background: "#101012", color: "#fff", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" },
  sidebar: { minHeight: "100vh", background: "#151518", borderRight: "1px solid #25252a", padding: "12px 10px", position: "sticky", top: 0 },
  ...
  roundButton: { width: 32, height: 32, borderRadius: 999, border: "1px solid #333", background: "#222", color: "#ccc" },
  ...
  promptBox: { position: "relative", width: "min(910px, 94%)", minHeight: 112, margin: "36px auto 16px", display: "grid", gap: 10, padding: "18px 20px 14px", borderRadius: 26, background: "#1d1d1c", border: "2px solid rgba(255,255,255,.16)", boxShadow: "0 22px 80px rgba(0,0,0,.5)" },
  ...
  askButton: { width: 42, height: 42, borderRadius: 999, border: 0, background: "#d7ff1f", color: "#111", fontWeight: 950, fontSize: 20, cursor: "pointer" },
  ...
  thumbIcon: { fontSize: 46, fontWeight: 900, color: "rgba(255,255,255,.9)" },
  cardFooter: { display: "grid", gridTemplateColumns: "42px 1fr auto", gap: 12, alignItems: "center", padding: 14 },
  cardIcon: { width: 42, height: 42, borderRadius: 12, display: "grid", placeItems: "center", background: "#25252b", color: "#d7ff1f" },
  cardTitle: { margin: 0, fontSize: 16 },
  cardDesc: { margin: "4px 0 0", color: "#9c9ca3", fontSize: 13 },
  tryButton: { border: 0, background: "#d7ff1f", color: "#111", borderRadius: 999, padding: "8px 12px", fontWeight: 950 },
};
